...
import com.example.casavideo.cmd.LowerThirdColorType;
import com.example.casavideo.util.DisplayUtil;
import com.example.casavideo.util.SharePreferenceUtil;
import com.example.casavideo.view.CycleColorImageView;
...
public class LowerThirdSettingFragment extends BottomSheetDialogFragment {
    ...
    private void onFragmentFinish(boolean save) {
        ...
        if (ZoomVideoSDK.getInstance().getSession() != null && zoomCanvas != null) {
            ZoomVideoSDKUser mySelf = ZoomVideoSDK.getInstance().getSession().getMySelf();
            mySelf.getVideoCanvas().unSubscribe(zoomCanvas);
        }
    }
}
